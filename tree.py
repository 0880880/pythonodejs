...
def print_tree(path, stats, max_files, ignore_list, prefix=''):
    ...
    dirs = [e for e in entries if os.path.isdir(os.path.join(path, e)) and e not in ignore_list]
...
def main():
    ...
    print(f"Scanning: {root} (skipping dirs > {args.max_files} files; ignoring: {ignore_list})\n")
